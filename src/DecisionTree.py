...
iid = str(gu_id)
log.basicConfig(filename="../src/logs/log/dectree/"+ str(gu_id) + ".log", filemode="a", format= "%(asctime)s \n %(message)s",datefmt='%d-%b-%y %H:%M:%S')
...
def fitness_func(solution, solution_idx):
    ...
    fit = arvRisco.fit(X_train_turbo, y_train)
    ...
    # log.warning(confusion_matrix(y_test,predictions))
    # log.warning(classification_report(y_test,predictions))
    # log.warning(accuracy_score(y_test,predictions))
    return fitness
...
def main():
    ...
    fit = arvRisco.fit(X_train_turbo_ga, y_train)
